...
# Always place the window to the center of the screen
root.geometry(
		f'{app_width}x{app_height}+{int(x)}+{int(y)}'
		)

# Disable window resizing, which also disables window snapping (problem ?)
...
# Press Start 2P
def font_1():
	# Update font inside text windget
	my_text.config(
...
# Initialize text-alignment Menu inside text Menu
align_menu = Menu(
		text_menu,
		tearoff=False
		)
# Add text-alignment Menu inside the text Menu
...
# Initialize language Menu inside the options Menu
language_menu = Menu(
		options_menu,
		tearoff=False
		)
# Add language Menu inside the options Menu
...
# Add help Menu command to help Menu
...
# Colorscheme to use when editor starts
...
my_text.config(
		...
		fg=text_color,
		selectbackground='yellow',
		selectforeground=textbox_color,
		)
